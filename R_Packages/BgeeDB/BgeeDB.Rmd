...
# What is BgeeDB?
BgeeDB is an R package to access the Bgee database of gene expression evolution. It allows to:  
** Retrieve processed expression values (RNA-Seq, Affymetrix, in situ hybridization, EST) for a given species and set of conditions
** Download cell level expression data (h5ad files)
** Download propagated calls of presence/absence of expression
** Perform TopAnat analyses (anatomical expression enrichment tests)
...
# Retrieve processed expression values
...
## Run the same analysis from sexually matured samles only
```{r}
